...
public class securityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http
			...
			.antMatchers("/", "/**", "/registerForm", "/login").permitAll()	// 제시된 요청은 접근을 허용한다.
			.antMatchers("/user/**").hasRole("USER")	// 제시된 요청은 USER 권한이 필요하다.
			.antMatchers("/admin/**").hasRole("ADMIN")	// 제시된 요청은 ADMIN 권한이 필요하다.
